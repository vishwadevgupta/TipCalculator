...
function getValues(){
  return {amount:num("amount"),tipPercent:Math.min(50,num("tip")),people:Math.max(1,Math.floor(num("people",1))),tax:num("tax"),service:num("service"),roundUp:$("roundUp").checked,unequal:$("unequalSplit").checked,customShare:num("customShare")};
}
...
async function copyResult(){try{await navigator.clipboard.writeText(resultText());toast("Result copied")}catch{toast("Copy isn't available in this browser")}}
async function shareResult(){const text=resultText();if(navigator.share){try{await navigator.share({title:"TipWise bill summary",text})}catch{}}else copyResult()}
...
$("amount").addEventListener("input",calculate);$("tip").addEventListener("input",calculate);$("people").addEventListener("input",()=>{$("people").value=Math.max(1,Math.floor(Number($("people").value)||1));calculate()});
["currency","tax","service","customShare"].forEach(id=>$(id).addEventListener("input",calculate));$("roundUp").addEventListener("change",calculate);$("unequalSplit").addEventListener("change",()=>{$("unequalPanel").style.display=$("unequalSplit").checked?"block":"none";calculate()});
$("minus").addEventListener("click",()=>changePeople(-1));$("plus").addEventListener("click",()=>changePeople(1));$("reset").addEventListener("click",reset);$("save").addEventListener("click",saveCalculation);$("copyResult").addEventListener("click",copyResult);$("shareResult").addEventListener("click",shareResult);
...
$("advancedToggle").addEventListener("click",()=>{const open=$("advancedPanel").classList.toggle("open");$("advancedToggle").classList.toggle("open",open);$("advancedToggle").setAttribute("aria-expanded",String(open))});
$("themeBtn").addEventListener("click",()=>{document.body.classList.toggle("light");const light=document.body.classList.contains("light");localStorage.setItem("tipwise-light",light?"1":"0");$("themeBtn").textContent=light?"🌙":"☀️"});
...
loadPreferences();if(localStorage.getItem("tipwise-light")==="1"){document.body.classList.add("light");$("themeBtn").textContent="🌙"}calculate();renderHistory();
